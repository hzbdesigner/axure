...
var 
	 ...
	 * 为旧设备提供fixed特性
	 */
	_fixedStyleHook = function(on) { if (!_q('.footFix')) return; if (typeof on == "undefined") on = true; var needFix = ('_needFixedStyle' in window) || (_env.ios && _env.version < 4.5) || (_env.android && _env.version < 3) || _env.meizu || (_env.blackberry && _env.version < 7) || !_testFixedSupport(); if (needFix) { if (on){ _hook1TO = window.setTimeout(_fixedStyleHelper, 200); window.addEventListener('scroll',_fixedStyleHelper); window.addEventListener('orientationchange',_fixedStyleHelper); window.addEventListener('touchmove',_fixedStyleHelper); window.addEventListener('touchend',_fixedStyleHelper); }else{ window.clearTimeout(_hook1TO); window.removeEventListener('scroll',_fixedStyleHelper); window.removeEventListener('orientationchange',_fixedStyleHelper); window.removeEventListener('touchmove',_fixedStyleHelper); window.removeEventListener('touchend',_fixedStyleHelper); } } }, _fixedStyleHelper = function(evt) { _forEach( _qAll('.footFix'), function(ftObj){ var $ft = ftObj, wh = window.innerHeight, wt = window.scrollY, ftop = MData($ft,'ffixTop'), fbtm = MData($ft,'ffixBottom'), fh; if ($ft){ try{ fh = $ft.clientHeight; $ft.style.position = 'absolute'; if (ftop){ $ft.style.top = wt+ftop*1+'px'; }else if (fbtm){ $ft.style.top = wt+wh-fbtm*1-fh+'px'; }else{ $ft.style.top = wt+wh-fh+'px'; } $ft.style.bottom = 'auto'; }catch(ex){} } });
	},
	/**
	 * 字符串trim
	 */
